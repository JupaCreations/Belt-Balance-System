/*
 Basic 2 loadcell program measuring belt tesion on a Rat-Rig V-Core 3-300 3D printer
 Optimized for Arduino Nano board
 Maximum loadcell weight 10 Kg
 HX711.DOUT  - we use digital pins
 HX711.PD_SCK - we use digital pins
 HX711 is set @ 10 samples/second (standard setting)
...
HX711 Scale1; // Loadcell 1
HX711 Scale2; // Loadcell 2

// HX711 circuit wiring
const int Loadcell1_DOUT_PIN = 6;
const int Loadcell1_SCK_PIN = 7;
...
int lastPulseCounter1, lastPulseCounter2 = 0; //-1;      //Previous value of count (initialized to ensure it's different when the sketch starts)
...
U8GLIB_SSD1306_128X64 u8g(U8G_I2C_OPT_NONE | U8G_I2C_OPT_DEV_0); //Display driver line I2C display
...
if (btnSELECT.wasReleased()) // reset to main screen and zero the loadcells (Tare)
{ //If the button was released
  PulseCounter2 = 0;
  PulseCounter3 = 0;
}
...
void draw(void)
{
  ...
  if (btnSELECT.pressedFor(3000)) and (Pulsecounter2 == 0)
  {
...
// Copyright by Jupa Creations The Netherlands
